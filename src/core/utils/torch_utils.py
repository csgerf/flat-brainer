...
def image_to_tensor(image: np.ndarray, dummy_channels_dim=True) -> torch.Tensor:
    ...
    Returns:
        torch.Torch tensor of [C,H,W] or [H,W] shape (dummy_channels_dim=False).
    """
...
def rgb_image_from_tensor(
    ...
    rgb_image = np.clip(rgb_image, a_min=min_pixel_value,
                        a_max=max_pixel_value)
...
def transfer_weights(model: nn.Module, model_state_dict: collections.OrderedDict):
    ...
    for name, value in model_state_dict.items():
        try:
            model.load_state_dict(collections.OrderedDict(
...
def describe_outputs(outputs: Union[Tensor, Dict[str, Tensor], Iterable[Tensor]]) -> Union[str, List[Dict], Dict[str, Any]]:
    """
