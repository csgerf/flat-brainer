...
def custom_flatten_dict(d, delimiter='_'):
    return stringify_unsupported(d)
    # result = {}
    # for key, value in d.items():
    #     if isinstance(value, dict):
    #         # Recursively flatten nested dictionaries
    #         nested_dict = custom_flatten_dict(value, delimiter)
    #         result.update({f"{key}{delimiter}{k}": v for k, v in nested_dict.items()})
    #     else:
    #         result[key] = value
    # return result
