...
__all__ = [
    ...
    "PolyLR",
    "contrib_scheduler_dict",
]

contrib_scheduler_dict = {
    "GradualWarmupScheduler": ["total_epoch", "after_scheduler", "multiplier"],
    "FlatCosineAnnealingLR": ["epochs", "min_lr_factor", "max_lr"],
    "OnceCycleLR": ["epochs", "min_lr_factor", "max_lr"],
    "CosineAnnealingLRWithDecay": ["epochs", "min_lr_factor", "max_lr"],
    "CosineAnnealingWarmRestartsWithDecay": ["epochs", "min_lr_factor", "max_lr"],
    "PolyLR": ["epochs", "min_lr_factor", "max_lr"],
}
...
class OnceCycleLR(LRScheduler):
    def __init__(self, optimizer: Optimizer, epochs: int, min_lr_factor: float = 0.05, max_lr: float = 1.0):
...
class CosineAnnealingLRWithDecay(LRScheduler):
    ...
    def __init__(self, optimizer: Optimizer, T_max: int, gamma: float, eta_min: float = 0, last_epoch: int = -1):
        self.gamma = gamma
        self.T_max = T_max
...
class PolyLR(LambdaLR):
    def __init__(self, optimizer: Optimizer, max_epoch: int, gamma: float = 0.9):
        def poly_lr(epoch):
...
class CosineAnnealingWarmRestartsWithDecay(CosineAnnealingWarmRestarts):
    def __init__(self, optimizer: Optimizer, T_0: int, T_mult=1, eta_min=0, last_epoch=-1, gamma=0.9):
...
class GradualWarmupScheduler(LRScheduler):
    ...
    def __init__(self, optimizer: Optimizer, multiplier: float, total_epoch: int,
                 after_scheduler: Optional[Union[LRScheduler, ReduceLROnPlateau]] = None):
    ...
    def step_reduce_lr_on_plateau(self, metrics: Any, epoch: Optional[int] = None):
        if epoch is None:
...
class FlatCosineAnnealingLR(LRScheduler):
    r"""Set the learning rate of each parameter group using a cosine annealing
    schedule, where :math:`\eta_{max}` is set to the initial lr and
    :math:`T_{cur}` is the number of epochs since the last restart in SGDR:
    .. math::
    ...
    def __init__(self, optimizer, T_max: int, T_flat: int, eta_min: float = 0, last_epoch: int = -1):
        self.T_max: int = T_max
        self.T_flat: int = T_flat
